...
public static partial class ImGui {
    ...
    public static void Joystick(Transform transform, float radius = 50f) {
        ImGui.PushID($"{transform.entity.get_path()}.joy");
    ...
    public static void Joystick2(Transform transform, float radius = 50f) {
        if(!joystick_data_list.TryGetValue(((IComponent)transform).get_path(), out var joystick_data)) {
            joystick_data = JoystickData.make();
            joystick_data_list.Add(((IComponent)transform).get_path(), joystick_data);
        }
