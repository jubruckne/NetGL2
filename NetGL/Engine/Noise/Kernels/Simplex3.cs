...
[SkipLocalsInit]
public readonly struct SimplexKernel3: IKernel {
    ...
    static Vector128<float> IKernel.evaluate(Vector128<float> x, Vector128<float> y) {
        ...
        var x1 = masked_sub_1_f32(x0, i1)
                 + vec4_float_G2;

        // float32v y1 = FS_NMaskedSub_f32( y0, float32v( 1.f ), i1 ) + float32v( G2 );
        var y1 = masked_sub_1_f32(y0, AdvSimd.Not(i1))
                 + vec4_float_G2;
        ...
        return vec4_float_38 * AdvSimd.FusedMultiplyAdd(AdvSimd.FusedMultiplyAdd(n1 * t1, n2, t2), n0, t0);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static Vector128<int> masked_add_i32(Vector128<int> left, Vector128<int> right, Vector128<int> mask)
        => Vector128.ConditionalSelect(mask, right, Vector128<int>.Zero) + left;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static Vector128<int> masked_n_add_i32(Vector128<int> left, Vector128<int> right, Vector128<int> mask)
        => Vector128.ConditionalSelect(AdvSimd.Not(mask), right, Vector128<int>.Zero) + left;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static Vector128<float> masked_sub_1_f32(Vector128<float> left, Vector128<float> mask)
        => left - Vector128.ConditionalSelect(mask, Vector128<float>.One, Vector128<float>.Zero);
