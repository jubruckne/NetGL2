...
[SkipLocalsInit]
public class SimplexKernel_Scalar: IKernel {
    private static int _seed;

    public static Vector128<float> evaluate(Vector128<float> x, Vector128<float> y)
        => Vector128.Create(
                            generate_2d(x.GetElement(0), y.GetElement(0)),
                            generate_2d(x.GetElement(1), y.GetElement(1)),
                            generate_2d(x.GetElement(2), y.GetElement(2)),
                            generate_2d(x.GetElement(3), y.GetElement(3))
                            );
