...
public static class Noise
{
    ...
    public static float[, ,] Calc3D(int width, int height, int depth, float scale)
    {
        float scale_i;
        float scale_j;
        ...
        for (var i = 0; i < width; i++){
            ...
            for (var j = 0; j < height; j++) {
                ...
                for (var k = 0; k < depth; k++)
                    values[i, j, k] = (Generate(scale_i, scale_j, k * scale) + 1f) * 0.5f;
            }
        }
    ...
    private static float Generate(in float x, in float y, in float z)
    {
